...
import { getEventDetailsShareUrl } from '@/utils';
...
export default function EventShareModal({ open, onOpenChange, eventId, eventTitle }) {
  ...
  const shareUrl = eventId ? getEventDetailsShareUrl(eventId) : '';
  const textSnippet = `Check out ${eventTitle || 'this event'} on SEC Nightlife: ${shareUrl}`;
  ...
  const sendToFriend = useCallback(
    async (friend) => {
      const body = `Check out this event: ${eventTitle || 'Event'}\n${shareUrl}`;
  ...
  const tw = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`${eventTitle || 'Event'} on SEC Nightlife`)}&url=${encodeURIComponent(shareUrl)}`;
